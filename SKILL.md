...
2️⃣ 分析后，仅把脱敏标签（如"布局者"）和城市级位置（非GPS）
   通过 OceanBus 匿名通道发往 L1 服务，获取气候、黄历数据
3️⃣ 在你的设备上创建 OceanBus 匿名身份凭据
   → 存储在 ~/.lucky-lobster/ 目录
   → 密码学随机地址，不关联你的真实身份
   → 随时可删除该目录，或者更换id，立即更新身份
...
**安装 ≠ 同意使用。安装仅表示你已阅读以下披露。** 首次使用（说"看运势"）时，系统会弹出显式同意提示——你需要输入"同意"后，运势分析才会开始。同意后写入 `~/.lucky-lobster/consent.json` 记录。

1. **本地人格分析**：Agent 会在你的设备本地读取对话记忆和会话风格进行分析。完整对话内容、项目名、人名永远不出设备。
2. **匿名数据传输**：分析完成后，仅脱敏后的人格标签（如"布局者"）、城市级位置（百万人级混淆）、年龄段（跨度≥5岁）通过 OceanBus L0 加密通道发往 L1 服务。
3. **匿名身份**：系统在 `~/.lucky-lobster/` 下创建 OceanBus 匿名凭证（密码学随机 OpenID）。OpenID 是随机字符串，不关联真实身份；随时删除该目录即生成新身份。
4. **定时推送**：仅在用户主动确认后启用。说"取消运势提醒"随时停止。
5. **推荐去重**：推荐功能将 OpenID 发往 L1 服务，仅用于记录"已推荐过的 skill"，避免重复推送。不用于跨会话追踪或个人画像。

OceanBus SDK 完全开源（MIT-0），可在 npmjs.com/package/oceanbus 审计。你不必信任我们——你可以验证代码。
